...
import 'database/database_1.dart';
import 'database/database_2.dart';
...
class _MainScreenState extends State<MainScreen> {
  ...
  Future<void> _loadDogs() async {
    final adopted = await _dbHelper.queryDogs(isAdopted: true);
    final givenAway = await _dbHelper.queryDogs(isAdopted: false);
    setState(() {
      myDogs = adopted.map((dog) => Dog.fromMap(dog)).toList();
      donatedDogs = givenAway.map((dog) => Dog.fromMap(dog)).toList();
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      bottomNavigationBar: NavigationBar(
        ...
        indicatorColor: Colors.blueGrey.withOpacity(0.3),
        ...
        destinations: const [
          NavigationDestination(
            selectedIcon: Icon(Icons.house, color: Colors.brown),
            icon: Icon(Icons.house_outlined, color: Colors.black),
            label: 'Home',
            tooltip: 'Go to Home Page',
          ),
          NavigationDestination(
            selectedIcon: Icon(Icons.favorite, color: Colors.brown),
            icon: Icon(Icons.favorite_border, color: Colors.black),
            label: 'Adopt',
            tooltip: 'View Adopted Dogs',
          ),
          NavigationDestination(
            selectedIcon: Icon(Icons.volunteer_activism, color: Colors.brown),
            icon: Icon(Icons.volunteer_activism_outlined, color: Colors.black),
            label: 'Give Away',
            tooltip: 'See Given Away Dogs',
          ),
          NavigationDestination(
            selectedIcon: Icon(Icons.pets, color: Colors.brown),
...
class DogSelector extends StatefulWidget {
  final Future<void> Function(Dog) addDog;
          ...
          builder: (ctx, snap) {
            ...
            if (snap.hasData && snap.data!.isNotEmpty) {
              final breeds =
                  snap.data!
                      .where(
                        (breed) => breed.title.toLowerCase().contains(
        ...
        if (isDogVisible) ...[
          ...
          FutureBuilder<DogPicture>(
            ...
            builder: (ctx, snap) {
              ...
              if (snap.hasError) return Text('Error: ${snap.error}');
              if (snap.hasData) {
                return Image.network(snap.data!.url, height: 300);
              }
...
class MyDogs extends StatelessWidget {
  final List<Dog> dogs;
  final Future<void> Function(Dog) donateDog;
  ...
  Widget build(BuildContext context) {
    return dogs.isEmpty
        ? const Center(
          child: Text('No dogs adopted yet!', style: TextStyle(fontSize: 18)),
        )
        : ListView.builder(
    ...
    return const Center(
      child: Padding(
        ...
        child: Text(
          ...
          "Lovely Dogs is more than just a dog adoption platform it’s a loving community that helps adorable dogs find their forever homes.\n"
...
class Dog {
  ...
  Dog({this.id, required this.title, required this.type, required this.pic});

  Future<Map<String, dynamic>> toMap() async {
    final imageUrl = (await pic).url;
    return {
      'id': id,
      'title': title,
      'breed': type.title,
      'imageUrl': imageUrl,
      'isAdopted': 1, // Default to adopted
    };
  }
  ...
  factory Dog.fromMap(Map<String, dynamic> map) {
    return Dog(
      id: map['id'],
      title: map['title'],
      type: DogBreed(title: map['breed']),
      pic: Future.value(DogPicture(url: map['imageUrl'])),
    );
  }
}
...
class DogPicture {
  ...
  factory DogPicture.fromData(Map<String, dynamic> data) =>
      DogPicture(url: data['message']);
}
...
Future<List<DogBreed>> getDogBreeds() async {
  ...
  if (resp.statusCode == 200) {
    ...
    return (json['message'] as Map<String, dynamic>).keys
        .map((k) => DogBreed.fromData(k))
        .toList();
  }
