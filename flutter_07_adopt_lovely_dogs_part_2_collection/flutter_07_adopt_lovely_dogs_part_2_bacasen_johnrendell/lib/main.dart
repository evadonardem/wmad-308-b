    ...
    return Column(
      ...
      children: [
        ...
        if (isDogVisible) ...[
          ...
          Text(currentDog!.title.toUpperCase(), style: const TextStyle(fontSize: 32, fontWeight: FontWeight.bold)),
          Text(currentDog!.type.title.toUpperCase(), style: const TextStyle(fontSize: 24)),
        ],
        const SizedBox(height: 20),
        Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            ElevatedButton(onPressed: adoptCurrentDog, child: const Text('Adopt')),
            const SizedBox(width: 15),
            ElevatedButton(onPressed: showNextDog, child: const Text('Next Dog')),
          ],
        ),
      ],
    );
...
class MyDogs extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return dogs.isEmpty
        ? const Center(child: Text('No dogs adopted yet!', style: TextStyle(fontSize: 18)))
        : ListView.builder(
...
class Dog {
  ...
  Future<Map<String, dynamic>> toMap() async {
    final imageUrl = await (await pic).url;
    return {
      'id': id,
      'title': title,
      'breed': type.title,
