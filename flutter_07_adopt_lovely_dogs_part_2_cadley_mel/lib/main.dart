    ...
    return Container(
      decoration: const BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [
            Color.fromARGB(255, 73, 177, 159), // Darker green at the top
            Color.fromARGB(255, 216, 245, 239), // Lighter green at the bottom
          ],
        ),
      ),
      child: Center(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  var breeds = snapshot.data!
                      .where((breed) => breed.name.toLowerCase().contains(searchQuery.toLowerCase()))
                      .toList();
                  return DropdownMenu(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            ...
            child: const Text('Give Away', style: TextStyle(color: Colors.green)),
          ),
        ],
      ),
    );
...
Future<List<Breed>> fetchBreeds() async {
  ...
  if (response.statusCode == 200) {
    ...
    throw Exception('Failed to fetch breeds: ${response.reasonPhrase}');
  }
}
